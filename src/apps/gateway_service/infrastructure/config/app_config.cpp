...
AppConfig AppConfig::load() {
    auto channelConfigPath =
        readString("CHANNEL_CONFIG_PATH", ".img/runtime/gateway_service/channel.state");
    const int defaultChannel = readInteger("DEFAULT_CHANNEL", 15);
    const int reconcileIntervalMilliseconds = readInteger("RECONCILE_INTERVAL_MS", 5000);

    const channel::domain::WirelessChannel validatedDefaultChannel{defaultChannel};
...
int AppConfig::defaultChannel() const {
